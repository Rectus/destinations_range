EntityFramework:CreateCppClassProxy("CVRApertureMultitool")


EntityClasses.CNewController = class(
	{
	...
	}, 
	{
	    ...
	    {
	        bases = {"root"},
	        description = "New controller entity test",
	        tags = 
	        {
	        	"publish",
	            "math",
	...
	EntityClasses.CVRApertureMultitool
)


function EntityClasses.CNewController:constructor()
	getbase( self ).constructor( self)

end

-- Functions
function EntityClasses.CNewController:Spawn(spawnKeys) 
    --getbase(self):Spawn(spawnKeys)
    --EnableDisable_Spawn(spawnKeys)
    self.value = spawnKeys:GetValue(startvalue)
end

function EntityClasses.CNewController:GetDebugName() 
	return "test debug name!"
end


function EntityClasses.CNewController:Add(toAdd) 
    self:Update(self.value, self.value + toAdd)
end


function EntityClasses.CNewController:Subtract(toSubtract) 
    self:Update(self.value, self.value - toSubtract)
end


function EntityClasses.CNewController:Update(oldValue, newValue) 
...
function EntityClasses.CNewController:GetValue() 
    return self.value
end
