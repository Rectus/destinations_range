...
local SPAWN_ITEMS = 
{
	...
	},
	{
		...
		{
			targetname = "",
			model = "models/props_misc/ruler_old.vmdl";
		}
	},
}


-- Any map specific player settings to load into the settings manager 
local MAP_PLAYER_DEFAULT_SETTINGS =
{
	...
	player_physics_collisionmode = {value = 1, type = "int"}; -- No head
...
local MAP_COMMANDS =
{
	--{cmd = "teleport_start", type = 0, text = "#Pause_Button_TeleportToStart"},
	--{cmd = "real_time", type = 2, def = 1, text = "Enable day-night cycle"},
	{cmd = "toggle_grid", type = 0, text = "Toggle Teleport Grid"},
	{cmd = "set_time_local", type = 0, text = "Local Time"},
	{cmd = "set_time_sunrise", type = 0, text = "Sunrise"},
	{cmd = "set_time_morning", type = 0, text = "Morning"},
	{cmd = "set_time_afternoon", type = 0, text = "Afternoon"},
	{cmd = "set_time_sunset", type = 0, text = "Sunset"},
	{cmd = "set_time_midnight", type = 0, text = "Midnight"},
...
function OnActivate()
	...
	if RandomFloat(0, 100) > 99  then
		EntFire(nil, "outdoor_table_cloth", "skin", "1", 0) 
	end
end
...
function OnMapCommand(this, data)
	...
	if data.cmd == "toggle_grid"
	then
		EntFire(nil, "teleport_area_toggle", "ToggleTest")
	...
	elseif data.cmd == "real_time"
	then
		local enabled = math.floor(data.val) == 1
		local ent = Entities:FindByName(nil, "sky_light")
		if ent then
			ent:GetPrivateScriptScope().SetTimeScale(enabled and 1 or 0)
		end	
		
	elseif data.cmd == "set_time_local"
	then
		local ent = Entities:FindByName(nil, "sky_light")
		if ent then
			ent:GetPrivateScriptScope().SetDayTimeToLocal()
		end	
	elseif data.cmd == "set_time_sunrise"
	then
		local ent = Entities:FindByName(nil, "sky_light")
		if ent then
			ent:GetPrivateScriptScope().SetDayTime(2.85)
		end	
	elseif data.cmd == "set_time_morning"
	then
		local ent = Entities:FindByName(nil, "sky_light")
		if ent then
			ent:GetPrivateScriptScope().SetDayTime(8.75)
		end
	elseif data.cmd == "set_time_afternoon"
	then
		local ent = Entities:FindByName(nil, "sky_light")
		if ent then
			ent:GetPrivateScriptScope().SetDayTime(13.0)
		end
	elseif data.cmd == "set_time_sunset"
	then
		local ent = Entities:FindByName(nil, "sky_light")
		if ent then
			ent:GetPrivateScriptScope().SetDayTime(21.0)
		end	
	elseif data.cmd == "set_time_midnight"
	then
		local ent = Entities:FindByName(nil, "sky_light")
		if ent then
			ent:GetPrivateScriptScope().SetDayTime(0.0)
		end	
	end
end
